...
function getTrackerDetectorRiskScore(trackerSignals = {}) {
  const value =
    typeof trackerSignals?.summary?.riskScore === "number"
      ? trackerSignals.summary.riskScore
      : typeof trackerSignals?.riskScore === "number"
...
function isFilteredNonPolicyPage(result) {
  ...
  const trackerRiskScore =
    typeof result?.trackerSignals?.summary?.riskScore === "number"
      ? result.trackerSignals.summary.riskScore
      : typeof result?.trackerSignals?.riskScore === "number"
...
export function computeFromHeuristic(result) {
  ...
  if (!normalized.isLikelyPolicyPage) {
    ...
    const trackerRiskScore = getTrackerDetectorRiskScore(trackerSignals);
