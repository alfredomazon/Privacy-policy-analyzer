...
const MAX_ANCHORS_TO_SCAN = 120;
const MAX_CANDIDATES_TO_VERIFY = 2;
const MAX_FETCHED_TEXT = 18000;
...
function extractPageMetaFromHtml(html, fallbackUrl = "") {
  ...
  main
    .querySelectorAll("script, style, noscript, svg, img, video, audio, header, footer, nav, aside, form")
  ...
  const text = norm((main.innerText || "").slice(0, MAX_FETCHED_TEXT));
...
async function fetchCandidatePage(absUrl) {
  try {
    ...
    const meta = extractPageMetaFromHtml(trimmedHtml, res.url || absUrl);
...
function collectCandidateLinks() {
  const anchors = Array.from(document.querySelectorAll("a[href]")).slice(0, MAX_ANCHORS_TO_SCAN);
  const candidates = [];
...
async function verifyCandidate(candidate) {
  ...
  const { finalScore, pageScore, candidateType } = scoreFetchedCandidate(
    fetched,
...
export async function findBestPolicyLink() {
  ...
  const topCandidates = initialCandidates.slice(0, MAX_CANDIDATES_TO_VERIFY);
  ...
  // Verify the best candidate first.
  const firstResult = await verifyCandidate(topCandidates[0]);
  checkedCandidates.push(firstResult);

  // Only verify one more candidate if the first one was weak or unusable.
  if (
    topCandidates[1] &&
    (!firstResult.fetched || firstResult.finalScore < 16)
  ) {
    const secondResult = await verifyCandidate(topCandidates[1]);
    checkedCandidates.push(secondResult);
  }
