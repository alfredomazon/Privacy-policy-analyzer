...
function scoreUrlQuality(urlText) {
  ...
  else if (/\/privacy-center\b/i.test(urlText)) score += 5;
...
function getCandidateOrigins(hostname = "") {
  ...
  if (parts.length >= 2) {
    origins.add(`https://${parts.slice(-2).join(".")}/`);
  }

  if (host === "docs.google.com" || host.endsWith(".google.com")) {
    origins.add("https://www.google.com/");
    origins.add("https://policies.google.com/");
  }

  if (host === "outlook.live.com" || host.endsWith(".microsoft.com") || host.endsWith(".live.com")) {
    origins.add("https://www.microsoft.com/");
    origins.add("https://privacy.microsoft.com/");
  }
...
function buildCommonPrivacyUrlsFromOrigins(origins = []) {
  const out = new Set();
...
function collectCandidateLinks(doc = document, baseUrl = window.location.href, sourceLabel = "page") {
  ...
  for (const a of anchors) {
    ...
    try {
      const abs = new URL(hrefRaw, baseUrl).toString();
      ...
      if (sourceLabel === "root-scan") score += 1;
      if (score <= 1) continue;
...
async function collectRootPageCandidates() {
  const origins = getCandidateOrigins(window.location.hostname);
  const all = [];
  ...
  for (const origin of origins.slice(0, 3)) {
    ...
    const links = collectCandidateLinks(fetched._doc, fetched.url, "root-scan");
    all.push(...links);
  }
...
async function probeCommonPrivacyUrls() {
  const origins = getCandidateOrigins(window.location.hostname);
  const urls = buildCommonPrivacyUrlsFromOrigins(origins).slice(0, 18);
  ...
  for (const url of urls) {
    ...
    const { finalScore, pageScore, candidateType, pageType, validation } = scoreFetchedCandidate(
      fetched,
      window.location.hostname
    );
  ...
  checkedCandidates.sort((a, b) => b.finalScore - a.finalScore);
...
export async function findBestPolicyLink() {
  ...
  const pageCandidates = collectCandidateLinks(document, window.location.href, "page");
  let combinedCandidates = [...pageCandidates];

  if (appShell.isAppShell || pageCandidates.length < 3) {
    const rootCandidates = await collectRootPageCandidates();
    combinedCandidates = dedupeCandidates([...combinedCandidates, ...rootCandidates]);
  }

  const topCandidates = combinedCandidates.slice(0, 6);

  if (!topCandidates.length || appShell.isAppShell) {
    const probed = await probeCommonPrivacyUrls();
  ...
  if (!best || best.finalScore < 16) {
    ...
    if (probed) {
      return {
        ...probed,
        checkedCandidates: [...checkedCandidates, ...probed.checkedCandidates].sort(
