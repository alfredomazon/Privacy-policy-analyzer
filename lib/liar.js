...
function computeBehaviorIntensity(trackerSignals = {}) {
  ...
  const riskLevel = String(
    trackerSignals?.summary?.riskLevel || trackerSignals?.riskLevel || ""
  ).toLowerCase();
  ...
  if (riskScore != null) {
    intensity = Math.min(intensity, Math.max(12, Math.round(riskScore * 1.4)));
  }
  if (trackerSignals?.summary?.routineOnly === true || riskLevel === "low") {
    intensity = Math.min(intensity, 24);
  } else if (riskLevel === "medium") {
    intensity = Math.min(intensity, 45);
  }
