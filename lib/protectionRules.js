...
const NOOP_REDIRECT_PATHS_BY_TYPE = {
  script: "/resources/noop.js",
  stylesheet: "/resources/noop.css",
  sub_frame: "/resources/noop-frame.html",
};
...
const CORE_RESOURCE_ALLOWLIST = [
  ...
  {
    hostPattern: /(^|\.)(mail\.google\.com|gmail\.com)$/i,
    resourceDomains: [
      "google.com",
      "gmail.com",
  ...
  {
    hostPattern: /(^|\.)(outlook\.live\.com|outlook\.office\.com|outlook\.office365\.com|mail\.live\.com)$/i,
    resourceDomains: [
      "live.com",
      "microsoft.com",
      "microsoftonline.com",
  ...
  {
    hostPattern: /(^|\.)mail\.yahoo\.com$/i,
    resourceDomains: ["yahoo.com", "yimg.com"],
  },
  {
    hostPattern: /(^|\.)(mail\.proton\.me|proton\.me|protonmail\.com)$/i,
    resourceDomains: ["proton.me", "protonmail.com"],
  },
  {
    hostPattern: /(^|\.)icloud\.com$/i,
    resourceDomains: ["icloud.com", "icloud-content.com", "apple.com"],
  },
];

const NETWORK_COMPATIBILITY_HOSTS = [
  {
    hostPattern: /(^|\.)youtube\.com$/i,
    reason: "streaming-video",
  },
  {
    hostPattern: /(^|\.)(mail\.google\.com|gmail\.com|outlook\.live\.com|outlook\.office\.com|outlook\.office365\.com|mail\.live\.com|mail\.yahoo\.com|mail\.proton\.me|proton\.me|protonmail\.com|icloud\.com)$/i,
    reason: "email-app",
  },
];
...
export function getNetworkProtectionCompatibility(hostname = "") {
  const pageHost = normalizeHost(hostname);
  const profile = NETWORK_COMPATIBILITY_HOSTS.find((entry) =>
    entry.hostPattern.test(pageHost)
  );
...
function dnrActionForResourceType(resourceType = "") {
  const noopPath = NOOP_REDIRECT_PATHS_BY_TYPE[resourceType];

  if (noopPath) {
    return {
      type: "redirect",
      redirect: {
        extensionPath: noopPath,
      },
    };
  }
...
function getDnrRuleResourceTypes(filter = {}) {
  return Array.from(new Set(filter.resourceTypes || [])).filter(Boolean);
}
...
export function buildDynamicProtectionRulesForHost({
  ...
  if (!getNetworkProtectionCompatibility(host).compatible) {
    return [];
  }
  ...
  const pushProtectionRules = (filter, priority = 1) => {
    if (Array.isArray(filter.urlIncludes) && filter.urlIncludes.length) {
      return;
    }

    for (const resourceType of getDnrRuleResourceTypes(filter)) {
      out.push({
        id: nextId,
        priority,
        action: dnrActionForResourceType(resourceType),
  ...
  if (rules.blockTrackers) {
    for (const filter of TRACKER_BLOCK_FILTERS) {
      pushProtectionRules(filter, filter.severity === "high" ? 3 : 2);
    }
  }

  if (rules.removeAds) {
    for (const filter of AD_BLOCK_FILTERS) {
      pushProtectionRules(filter, 3);
    }
  }
