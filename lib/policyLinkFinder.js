...
function isSameBrandPolicyHost(candidateHost = "", sourceHost = "") {
  if (sameRegistrableDomain(candidateHost, sourceHost)) return true;

  const sourceBrand = getDomainBrandToken(sourceHost);
  const candidateBrand = getDomainBrandToken(candidateHost);

  return (
    !!sourceBrand &&
    sourceBrand.length >= 4 &&
...
export function rankPolicyLinkCandidates(
  ...
  for (const link of links) {
    ...
    try {
      ...
      if (
        ["meta", "script", "structured-data"].includes(source) &&
        !isSameBrandPolicyHost(urlObj.hostname, sourceHost)
      ) {
        continue;
      }
