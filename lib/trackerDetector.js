...
function collectFingerprintingHints() {
  ...
  // Only return fingerprinting hints when evidence is stronger than a casual canvas/webgl mention.
  const shouldFlag =
    strongHits.length >= 1 || keywordHits.length >= 3;
...
function deriveBehaviorProfile({
  ...
  // Sharing should require stronger evidence than "three third-party resources exist".
  const sharing =
  ...
  // Form fields are useful context, but should not overstate suspicious behavior.
