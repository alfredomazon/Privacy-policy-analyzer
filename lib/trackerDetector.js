...
function trackerImpactReason(hit = {}) {
  const purpose = String(hit?.purpose || "").toLowerCase();
  const category = String(hit?.category || "").toLowerCase();

  if (hit?.routineCommerce === true) return "routine_commerce";
  if (/session replay|heatmap/.test(purpose)) return "session_replay";
  if (/social advertising|advertising|ad conversion/.test(purpose)) {
    return "cross_site_ads";
  }
  if (category === "sharing") return "cross_site_ads";
  if (/analytics|measurement|tag manager/.test(purpose)) {
    return "analytics_measurement";
  }

  return "known_tracker";
}

function storageImpactReason(signal = {}) {
  const label = String(signal?.label || "");
  const category = String(signal?.category || "").toLowerCase();

  if (signal?.routineCommerce === true) return "routine_storage";
  if (/fingerprint/i.test(label)) return "fingerprinting_storage";
  if (category === "sharing") return "persistent_ad_id";
  if (category === "identifiers") return "persistent_browser_id";

  return "browser_storage";
}

function formImpactReason(signal = {}) {
  const category = String(signal?.category || "").toLowerCase();

  if (signal?.routineCommerce === true) return "routine_form";
  if (category === "sensitive") return "sensitive_form";
  if (category === "location") return "location_form";
  if (category === "financial") return "financial_form";
  if (category === "identifiers") return "identifying_form";

  return "data_entry";
}

function thirdPartyImpactReason(resource = {}) {
  if (resource?.routineCommerce === true) return "routine_commerce";
  if (resource?.knownTracker === true) return "known_tracker";
  if (resource?.likelyBenign === true) return "benign_resource";
  if (String(resource?.kind || "").startsWith("unknown_")) return "unknown_third_party";

  return "third_party_resource";
}

function withTrackerImpactReason(hit = {}) {
  return {
    ...hit,
    impactReason: hit.impactReason || trackerImpactReason(hit),
  };
}

function withStorageImpactReason(signal = {}) {
  return {
    ...signal,
    impactReason: signal.impactReason || storageImpactReason(signal),
  };
}

function withFormImpactReason(signal = {}) {
  return {
    ...signal,
    impactReason: signal.impactReason || formImpactReason(signal),
  };
}

function withThirdPartyImpactReason(resource = {}) {
  return {
    ...resource,
    impactReason: resource.impactReason || thirdPartyImpactReason(resource),
  };
}
...
function summarizeVendors(trackerHits = []) {
  ...
  for (const hit of trackerHits) {
    ...
    if (hit?.impactReason) existing.impactReasons.add(hit.impactReason);
  ...
  return Array.from(byVendor.values())
    .map((item) => ({
      ...
      impactReasons: Array.from(item.impactReasons).slice(0, 4),
    }))
    .sort((a, b) => {
      const severityDiff = severityRank(b.severity) - severityRank(a.severity);
      if (severityDiff) return severityDiff;
...
function buildSummary({
  ...
  const impactReasons = uniqBy(
    allSignals.map((signal) => signal?.impactReason).filter(Boolean),
    (x) => x
  ).slice(0, 8);
...
export function detectTrackerSignals({ networkSignals = [] } = {}) {
  ...
  const trackerHits = uniqBy(
    sortSignalsByStrength(
      [...scriptAndIframe.trackerHits, ...networkTrackerHits].map((hit) =>
        withTrackerImpactReason(normalizeCommerceTrackerHit(hit, commerceContext))
      )
    ),
    (x) => `${x.id}|${x.sourceType}|${x.requestType}|${x.hostname}|${x.url}`
  ).slice(0, MAX_TRACKER_HITS);
  const storageSignals = sortSignalsByStrength(
    rawStorageSignals.map((signal) =>
      withStorageImpactReason(normalizeCommerceStorageSignal(signal, commerceContext))
    )
  );
  const formSignals = sortSignalsByStrength(
    rawFormSignals.map((signal) =>
      withFormImpactReason(normalizeCommerceFormSignal(signal, commerceContext))
    )
  );
  const fingerprintingHints = collectFingerprintingHints().map((hint) => ({
    ...hint,
    impactReason: hint.impactReason || "fingerprinting",
  }));
