...
const STRONG_FINGERPRINTING_KEYWORDS = [
  /fingerprint/i,
  /deviceMemory/i,
  /hardwareConcurrency/i,
  /navigator\.plugins/i,
...
const MAX_SCRIPT_IFRAME_SCAN = 80;
const MAX_STORAGE_KEYS_PER_STORE = 40;
const MAX_FORM_FIELDS = 60;
const MAX_INLINE_SCRIPTS = 20;
const MAX_INLINE_SCRIPT_CHARS = 40000;
const MAX_HTML_SCAN_CHARS = 50000;
...
function collectScriptAndIframeSignals() {
  ...
  const scripts = Array.from(document.querySelectorAll("script[src]"))
    .slice(0, MAX_SCRIPT_IFRAME_SCAN)
    .map((el) => el.src)
    .filter(Boolean);
...
function collectStorageSignals() {
  ...
  function scanStorage(storageObj, storageType) {
    ...
    const limit = Math.min(storageObj.length, MAX_STORAGE_KEYS_PER_STORE);
...
function collectFormSignals() {
  ...
  return uniqBy(
    hits,
    (x) =>
      `${x.category}|${x.label}|${x.fieldType}|${x.name}|${x.id}|${x.autocomplete}`
  );
}
...
function collectFingerprintingHints() {
  ...
  const htmlText = htmlRoot?.innerHTML || "";
  const combined = `${inlineScripts}\n${htmlText.slice(0, MAX_HTML_SCAN_CHARS)}`;

  const keywordHits = [];

  for (const pattern of FINGERPRINTING_KEYWORDS) {
    if (pattern.test(combined)) {
      keywordHits.push(pattern.source);
    }
  }

  const strongHits = STRONG_FINGERPRINTING_KEYWORDS.filter((pattern) =>
    pattern.test(combined)
  ).map((pattern) => pattern.source);

  const shouldFlag = strongHits.length >= 1 || keywordHits.length >= 3;

  if (!shouldFlag) return [];

  return uniqBy(keywordHits, (x) => x).map((keyword) => ({
    keyword,
    category: "tracking",
    severity: "high",
...
function deriveConfidence({
  ...
  const hasHighSeverityForm = formSignals.some((x) => x.severity === "high");
  const hasSharingStorage = storageSignals.some(
    (x) => String(x?.category || "").toLowerCase() === "sharing"
  );

  if (fingerprintingHints.length > 0) return "high";
  if (hasHighSeverityTracker) return "high";
  ...
  if (hasHighSeverityForm && trackerHits.length > 0) return "medium";
