...
const CATEGORY_EXPECTED_USE_PATTERNS = {
  ...
  sharing: [
    /\bservice providers?.*(?:perform services|on our behalf|operate the service|process payments|fulfill orders?)\b/i,
    /\bvendors?.*(?:perform services|on our behalf|operate the service|process payments|fulfill orders?)\b/i,
...
function getUseContext(rule, text = "") {
  ...
  const secondaryUse = !saleDenial && hasAny(t, SECONDARY_USE_PATTERNS);
  const highRiskSecondaryUse =
    !saleDenial && hasAny(t, HIGH_RISK_SECONDARY_USE_PATTERNS);
  const outsideSources = hasAny(t, OUTSIDE_DATA_PATTERNS);
  const highRiskOutsideSources = hasAny(t, HIGH_RISK_OUTSIDE_DATA_PATTERNS);
  ...
  const preciseLocation = /\bprecise (?:location|geolocation)\b|\bgps location\b/i.test(t);
  ...
  const saleOrSharing =
    /\bsell or share\b|\bsell (?:your )?(?:personal )?information\b|\bsale of (?:personal )?information\b|\bvaluable consideration\b/i.test(t);
...
function priorityReasonForContext(rule, ctx = {}) {
  ...
  if (category === "sale") return "sale-or-sharing";
